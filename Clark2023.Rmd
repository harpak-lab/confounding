...
corrected_vs_uncorrected_cors <- ds_osd_m %>% 
  dplyr::select(relationship_rename, n, cor, cor_mean) %>%
  pivot_longer(cor:cor_mean) %>% 
  mutate(name=case_match(name,
                         "cor" ~ "Clark (2023) estimation;\n using different sample variances and \n susceptible to pseudo-replication",
                         "cor_mean" ~ "Unbiased estimation")) %>%
...
fig_s8a <- fig_s8_dat %>% 
  ...
  geom_brace(aes(c(0.8, 1.2), c(200, 10)), inherit.data = F, rotate = 180) +
  geom_brace(aes(c(1.8, 2.2), c(200, 10)), inherit.data = F, rotate = 180) +
  scale_x_continuous(breaks=1:9) +
  scale_color_manual(values = c("#d8b365", "#5ab4ac")) +  # divergent color scheme
  # single legend for color and shape
  xlab("Degree of relatedness (n)") +
  annotate("text", x = 1, y = 50, label = "using all pairs \n as in Clark (2023) estimation", hjust = 0, color = "#d8b365", size = 6) +
  annotate("text", x = 6, y = 125, label = "using only \nunique individuals", hjust = 0, color = "#5ab4ac", size = 6) +
...
occ_dist_raw <- bind_rows(rels_occ %>%
                            dplyr::filter(per1780==1 & !is.na(occ0) & !is.na(occ1) & relationship!="random") %>% 
...
occ_dist_pr <- bind_rows(rels_occ %>%
                           dplyr::filter(per1780==1 & !is.na(occ0) & !is.na(occ1) & relationship!="random") %>% 
                           dplyr::select(relationship, pid=pid0, occ=occ0, wealth=lwealth_0, byr=byr0),
                         rels_occ %>%
                           dplyr::filter(per1780==1 & !is.na(occ0) & !is.na(occ1) & relationship!="random") %>% 
