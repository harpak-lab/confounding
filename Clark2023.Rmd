...
# See README for data provenance
...
```{r lit long}
####
## Make long literacy df (one individual per row)
####
...
relationship_vec <- c("Full sibling", "Child", "Sibling-rem", "Grandchild", "Cousin", "Cousin-rem", "Cousin2", "Cousin2-rem", "Cousin3", "Cousin3-rem", "Cousin4")
...
relationship_rels_occ_vec <- c("sons", "father-son", "siblings-rem", "grandson", "cousin", "cousin-rem", "cousin2", "cousin2-rem", "cousin3", "cousin3-rem", "cousin4")
...
relationship_rename_vec <- c("Full siblings", "Parent-child", "Avuncular", "Grandparent-grandchild", "1st Cousins", "1st Cousins once removed", "2nd Cousins", "2nd Cousins once removed", "3rd Cousins", "3rd Cousins once removed", "4th Cousins")


# make df with relationship, cor, and `n` and `d_lin` parameters from Eq 3
relations.df <- data.frame(relationship = relationship_vec,
                           relationship_rename = relationship_rename_vec,
                           relationship_rels = relationship_rels,
                           relationship_rels_occ = relationship_rels_occ_vec,
                           relationship_rels_lit = relationship_rels_lit,
                                    n = c(1,0,2,1,3,4,5,6,7,8,9), # genealogical distance
...
occstat_1780 <- rels_occ %>%
  filter(per1780 == 1) %>% # filter to records born 1780-1859, as in C23
  filter(relationship != "random") %>% # only keep pairs with known relationship; exclude random pairs
...
occstat_1860 <- rels_occ %>%
  filter(per1860 == 1) %>% # filter to records born after 1859, as in C23
...
ggplot(correlations.df, aes(x = relationship_rename, y = cor, color = source)) +
...
#### Correlations look good with the exception of full sibling higher education. We alerted Dr. Clark of this discrepancy but did not receive any corrected data.

```


# Fig 2a - Paternal wealth

Calculate correlations between paternal wealth and status measures; plot occStat ~ paternal wealth; plot cor(occStat) ~ cor(paternal wealth)

```{r Table S2 correlations}
#### Calculate correlations between paternal wealth and status measures

# Create individual-level df with data on occupational status and paternal wealth
inds_wealth <- inds %>%
...
# Calculate mean correlation between an individual's wealth and their status, across status measures
mean(c(
...
# Calculate mean correlation between an individual's father's wealth and the individual's status, across status measures
mean(c(
...
```{r df}
# Make df for Fig 2a inset

inds_wealth_1780 <- inds_wealth %>%
  filter(byr < 1860) %>%
#  filter(byr > 1859) %>% # To check correlations for later birth cohort
  filter(!is.na(wealth_fath) & !is.na(occ))
  # filter(between(wealth_fath, # to exclude outliers
  #                quantile(wealth_fath, 0.25, na.rm = TRUE) - 1.5 * IQR(wealth_fath, na.rm = TRUE),
  #                quantile(wealth_fath, 0.75, na.rm = TRUE) + 1.5 * IQR(wealth_fath, na.rm = TRUE)))


# Correlation between individual's occupational status and their father's wealth
...
```{r downsampling}
#### Downsample to one record per lineage (nid) for 1000 bootstrap iterations
...
#write.csv(ds_occstat, "ds_occstat.csv", row.names = F)
#ds_occstat <- read.csv(paste0(data_loc, "ds_occstat.csv"))



# Calculate pair correlations, without correcting for pseudoreplication
...
# Make df that includes corrected and uncorrected pair correlations
ds_osd_m <- ds_occstat %>%
  group_by(relationship) %>%
...
```{r inds v pairs}
# Calculate how many unique individuals and how many reported pairs are present in each relationship class
pr_ind <- rels_occ %>% 
...
### Heatmap
```{r occStat 1780}
#### Generate heatmap to visualize extent of pseudoreplication
...
# Check for number of unique individuals
unique_check <- inds_occStat_1780 %>%
  filter(relationship == "cousin4") %>%
  distinct(pid)

# get number duplicated
dups <- inds_occStat_1780 %>%
  group_by(relationship, pid) %>%
  summarise(count = n(), .groups = "drop") %>%
  group_by(relationship) %>%
  summarise(
    unique_pids = sum(count == 1),
    repeated_pids = sum(count > 1)
  ) %>%
  mutate(perc_dup = repeated_pids / (unique_pids + repeated_pids))
...
# calculate pseudo-rep statistic (the average number of times an individual is included as a member of a pair) for each combination of relationships
...
left_join(relations.df %>% dplyr::select(relationship_rename, relationship_rels_occ), by = c("relationship_2" = "relationship_rels_occ")) %>%
  ...
  ungroup() %>%
  select(-row_num)  # remove the helper column 'row_num'
...
# Figs 2c / S13 - Social mobility

Here we look at patterns of social mobility through time, by analyzing familial correlations through time for various status measures.

## Corrected for pseudoreplication

```{r functions}
# functions used below

# Function to calculate mean and confidence limits
...
Now we will (partially) correct for pseudoreplication by running 500 bootstrap iterations of correlation estimation, sampling one record per lineage (nid) each iteration. We'll do this for multiple status measures: occupational status (raw index and percentile rank), higher education, literacy, and wealth.

```{r occStat}
#### Occupational status

# bin records into birth year cohorts
...
#write.csv(results_df_occ, "data/results_df_occ.csv", row.names = F)
...
#### Occupational status (percentile rank)
...
#write.csv(results_df_occ_perc, "data/results_df_occ_perc.csv", row.names = F)
...
occStat_percentile.gg <- results_df_occ_perc %>%
  ...
  labs(x = "Child birth year",
       y = "Correlation") +
  annotate("text", x = 5, y = 0.65, label = "italic('Occupational status\n(percentile rank)')", size = 8, parse = TRUE) +
  theme_classic() +
...
#write.csv(results_df_ded, "data/results_df_ded.csv", row.names = F)
...
#write.csv(results_df_lit, "data/results_df_lit.csv", row.names = F)
...
# make df and bin by birth cohort
mobility_wealth <- rels_occ %>%
  left_join(wealth_long, by = join_by(pid0 == pid)) %>%
  rename(wealth0 = lwealth) %>%
  left_join(wealth_long, by = join_by(pid1 == pid)) %>%
  rename(wealth1 = lwealth) %>%
  filter(!is.na(wealth0) & !is.na(wealth1)) %>%
  filter(relationship == "father-son") %>%
  mutate(byr1_bin = cut(byr1, 
                      breaks = seq(1780, 1920, by = 20) - 1,
                      labels = paste0(seq(1780, 1920 - 20, by = 20), "-", seq(1799, 1920 - 1, by = 20)),
...
# Pre-allocate a list to store the results
results_list_wealth <- vector("list", n_iter)
...
for(i in seq_len(n_iter)) {
  ...
  single_run_result <- mobility_wealth %>%
#    filter(relationship == "father-son") %>%
    group_by(byr1_bin, nid) %>%
    slice_sample(n = 1) %>%
    group_by(byr1_bin) %>%
    filter(n() > 10) %>% # must have more than 10 data points (to avoid spurious correlations)
    do(tidy(cor.test(.$wealth0, .$wealth1, method = "pearson"))) %>%
    dplyr::select(byr1_bin, estimate, conf.low, conf.high)
  
  # Store the result in the list
  results_list_wealth[[i]] <- single_run_result
...
# Combine all the results into a single data frame
results_df_wealth <- do.call(rbind, results_list_wealth)

quantile(results_df_wealth[results_df_wealth$byr1_bin == "1900-1919",]$estimate, probs = 0.975, na.rm = TRUE)

#write.csv(results_df_wealth, "data/results_df_wealth.csv", row.names = F)
#results_df_wealth <- read.csv(paste0(data_loc, "results_df_wealth.csv"))


means_wealth <- results_df_wealth %>%
  group_by(byr1_bin) %>%
  summarise(estimate_mean = mean(estimate)) %>%
  mutate(group = "wealth")
...
# Pre-allocate a list to store the results
results_list_wealth_perc <- vector("list", n_iter)
...
#    filter(relationship == "father-son") %>%
    ...
    group_by(byr1_bin) %>%
    filter(n() > 10) %>% # must have more than 10 data points (to avoid spurious correlations)
    mutate(wealth0_rank = calc_percentile_rank(wealth0),
           wealth1_rank = calc_percentile_rank(wealth1)) %>%
    do(tidy(cor.test(.$wealth0_rank, .$wealth1_rank, method = "pearson"))) %>%
    dplyr::select(byr1_bin, estimate, conf.low, conf.high)
  
  # Store the result in the list
  results_list_wealth_perc[[i]] <- single_run_result
...
# Combine all the results into a single data frame
results_df_wealth_perc <- do.call(rbind, results_list_wealth_perc)


#write.csv(results_df_wealth_perc, "data/results_df_wealth_perc.csv", row.names = F)
#results_df_wealth_perc <- read.csv(paste0(data_loc, "results_df_wealth_perc.csv"))


means_wealth_perc <- results_df_wealth_perc %>%
  group_by(byr1_bin) %>%
  summarise(estimate_mean = mean(estimate)) %>%
  mutate(group = "wealth")

wealth_percentile.gg <- results_df_wealth_perc %>%
  ggplot() +
  geom_line(data = means_wealth_perc, aes(x = byr1_bin, y = estimate_mean, group = group)) +
  stat_summary(aes(x = byr1_bin, y = estimate), fun.data = custom_fun) +
  labs(x = "Child birth year",
       y = "Correlation") +
  annotate("text", x = 4, y = 0.57, label = "italic('Wealth (percentile rank)')", size = 8, parse = TRUE) +
...
```{r all relatives}
# Get correlations in occupational status and higher education for all relationship classes
...
## Correlations not Corrected for pseudoreplication

```{r wealth}
# Divide into birth year cohorts
rels_wealth_binned <- rels_occ %>%
  left_join(wealth_long, by = join_by(pid0 == pid)) %>%
  rename(wealth0 = lwealth) %>%
  left_join(wealth_long, by = join_by(pid1 == pid)) %>%
  rename(wealth1 = lwealth) %>%
...
# get df with both occupational status and education
...
mobility_wealth.gg <- rels_wealth_binned %>%
  filter(relationship == "father-son") %>%
  mutate(trait = "wealth") %>%
  ggplot(aes(x = byr1_bin, y = estimate, ymin = conf.low, ymax = conf.high, group = trait)) +
  geom_pointrange() +
  ...
  labs(x = "Child birth year",
       y = "Correlation") +
  annotate("text", x = 5, y = 0.6, label = "italic('Wealth')", size = 8, parse = TRUE) +
  theme_classic() +
  theme(axis.text.x = element_text(angle = 45, hjust = 1, size = 14),
        axis.text.y = element_text(size = 14),
        axis.title = element_text(size = 18))
...
mobility_noCorrect.gg <- plot_grid(mobility_occ.gg, mobility_edu.gg, mobility_lit.gg, mobility_wealth.gg, nrow = 4, rel_heights = c(0.9, 1.1, 1.1, 1), labels = c("(d)", "(e)", "(f)", "(g)"), label_size = 18, label_x = c(-0.04, -0.04, -0.04, -0.04))
...
```{r}
plot_grid(plot_grid(mobility_all_rels.gg, occStat_percentile.gg, wealth_percentile.gg, nrow = 3, labels = c("(a)", "(b)", "(c)"), rel_heights = c(1, 0.5, 0.5), label_size = 18, label_y = c(1,1.04,1.04)), mobility_noCorrect.gg, nrow = 1, labels = c("", ""), rel_widths = c(1, 0.7))
...
```{r confound panel a}
#################################
### Inset showing relationship between individual's occupational status and their father's wealth
#################################
confound_inset.gg <- ggplot(inds_wealth_1780, aes(x = wealth_fath, y = occ)) +
  geom_point(alpha = 0.2, color = "#91bfdb") +
  geom_smooth(method = "lm", se = F, color = "#08519c") +
  labs(x = "Paternal wealth", y = "Occupational status", subtitle = "Across individuals") +
  theme_classic() +
  #annotate("text", x = -3, y = 5, label = "b = 0.43", size = 8) +
  annotate("text", x = 6, y = 12, label = "italic(r)*' = 0.72'", size = 6, parse = T, color = "#08519c") +
...
#################################
### Main plot showing the correlation in paternal wealth versus correlation in occupational status, across relationship classes
#################################

# get father-son df to lookup paternal wealth
fathers_sons <- rels_occ %>%
  filter(relationship == "father-son") %>%
  mutate(pid_father = pid0,
         pid_son = pid1)

# assign paternal wealth info
rels_occ_fam_wealth <- rels_occ %>%
  ...
  # filter(between(pid0_father_wealth, 
  #                quantile(pid0_father_wealth, 0.25, na.rm = TRUE) - 1.5 * IQR(pid0_father_wealth, na.rm = TRUE),
  #                quantile(pid0_father_wealth, 0.75, na.rm = TRUE) + 1.5 * IQR(pid0_father_wealth, na.rm = TRUE)) &
  #          between(pid1_father_wealth, 
  #                quantile(pid1_father_wealth, 0.25, na.rm = TRUE) - 1.5 * IQR(pid1_father_wealth, na.rm = TRUE),
  #                quantile(pid1_father_wealth, 0.75, na.rm = TRUE) + 1.5 * IQR(pid1_father_wealth, na.rm = TRUE)))


# get correlations for paternal wealth and occupational status, for each relationship class
...
# correlation between occStat correlation and paternal wealth correlation, across relationship classes
cor.test(predict.df$cor_occ, predict.df$cor_father_wealth)

set.seed(42) # for plot labels

confound_main.gg <- ggplot() +
  geom_point(data = predict.df, aes(x = cor_father_wealth, y = cor_occ, label = relationship_rename), size = 4) +
  geom_smooth(data = predict.df, aes(x = cor_father_wealth, y = cor_occ), method = "lm", se = F, color = "black") +
  ...
  geom_text_repel(data = predict.df, aes(x = cor_father_wealth, y = cor_occ, label = relationship_rename), size = 4, box.padding = 0.75, nudge_x = -.05) +
  labs(x = "Correlation in paternal wealth", y = "Correlation in occupational status", title = "Example of confounding") +
  annotate("text", label = "italic(r)*' = 0.91'", x = 0.2, y = 0.6, size = 8, parse = T) +
...
confound.gg <- ggdraw(confound_main.gg) +
  draw_plot(confound_inset.gg, x = 0.53, y = 0.11, 0.46, 0.4) 
...
#  mutate(relationship_rename = fct_recode(relationship_rename, "Siblings once removed" = "Uncle-nephew", "Grandparent-grandchild" = "Grandparent-Grandchild")) %>%
  ...
  mutate(relationship_rename = fct_relevel(relationship_rename, relationship_rename_vec)) %>%
    mutate(n_adj = ifelse(relationship_rename == "Parent-child", 0.85, ifelse(relationship_rename == "Full siblings", 1.15, ifelse(relationship_rename == "Avuncular", 1.85, ifelse(relationship_rename == "Grandparent-grandchild", 2.15, n))))) 
...
pseudo.gg <- corrected_vs_uncorrected_cors %>%
  ...
  geom_text(data=corrected_vs_uncorrected_cors %>%
            ...
            hjust = 0.05,
            lineheight = 0.8)+
  labs(x = "Degree of relatedness (n)", y = "Correlation in occupational status", title = "Statistical artifacts influence estimates") +
  scale_x_continuous(breaks = 1:9) +
  scale_y_continuous(expand = expansion(mult = c(0.02, 0.1)), breaks = c(0.0, 0.2, 0.4, 0.6)) + 
...
#  stat_brace(aes(c(0.8, 1.2), c(-0.12, -0.08)), outside = F, rotate = 180) +
#  stat_brace(aes(c(1.8, 2.2), c(-0.12, -0.08)), outside = F, rotate = 180) +
  theme_classic() +
...
#        axis.text.x=element_blank(),
        ...
        plot.title = element_text(size = 20, face = "bold", vjust = 9),
#        legend.box.background = element_rect(linewidth = 1),
        plot.margin = margin(0.8, 0.1, 0.2, 0.1, "in")) +
...
```{r mobility panel c}
mobility_occ.gg <- results_df_occ %>%
  filter(relationship == "father-son") %>%
  ggplot() +
  ...
  labs(x = "",
       y = "Father-son") +
#  annotate("text", x = 7, y = 0.35, label = "italic('Occupational status')", size = 5, parse = TRUE, hjust = 1) +
  scale_y_continuous(labels = c("0.4", "", "0.6", "", "0.8"), breaks = c(0.4, 0.5, 0.6, 0.7, 0.8)) +
  annotation_custom(grid::textGrob(expression("Occupational status"), x = 0.95, y = 0.9, hjust = 1, gp = grid::gpar(fontsize = 14))) +
  theme_classic() +
...
mobility_ded.gg <- results_df_ded %>%
  filter(relationship == "father-son") %>%
  ggplot() +
  ...
  labs(x = "Offspring birth year",
       y = "Father-son") +
#  annotate("text", x = 1, y = 0.05, label = "italic('Higher education')", size = 5, parse = TRUE, hjust = 0) +
  annotation_custom(grid::textGrob(expression("Higher education"), x = 0.95, y = 0.9, hjust = 1, gp = grid::gpar(fontsize = 14))) +
  theme_classic() +
  scale_y_continuous(breaks = c(0, 0.2, 0.4, 0.6, 0.8, 1), labels = c("0.0", "", "0.4", "", "0.8", "")) +
  theme(axis.text.x = element_text(angle = 50, hjust = 1, size = 14),
        axis.text.y = element_text(size = 14),
        axis.title = element_blank(),
        plot.margin = margin(5.5,5.5,5.5,0, "pt"))
...
mobility_lit.gg <- results_df_lit %>%
  filter(Relationship == "parent-child") %>%
  ggplot() +
  ...
  labs(x = "",
       y = "Parent-child") +
#  annotate("text", x = 1, y = 0, label = "italic('Literacy')", size = 5, parse = TRUE, hjust = 0) +
  annotation_custom(grid::textGrob(expression("Literacy"), x = 0.95, y = 0.9, hjust = 1, gp = grid::gpar(fontsize = 14))) +
   scale_y_continuous(limits = c(-0.1, 0.8), breaks = c(0, 0.2, 0.4, 0.6, 0.8), labels = c("0.0", "", "0.4", "", "0.8")) +
  theme_classic() +
  theme(axis.text.x = element_text(angle = 50, hjust = 1, size = 14),
        axis.text.y = element_text(size = 14),
        axis.title = element_blank(),
...
mobility_wealth.gg <- results_df_wealth %>%
  ggplot() +
  geom_line(data = means_wealth, aes(x = byr1_bin, y = estimate_mean, group = group)) +
  stat_summary(aes(x = byr1_bin, y = estimate), fun.data = custom_fun) +
  labs(x = "Offspring birth year",
       y = "Father-son") +
#  annotate("text", x = 1, y = 0.2, label = "italic('Wealth')", size = 5, parse = TRUE, hjust = 0) +
  annotation_custom(grid::textGrob(expression("Wealth"), x = 0.95, y = 0.9, hjust = 1, gp = grid::gpar(fontsize = 14))) +
   scale_y_continuous(breaks = c(0, 0.2, 0.4, 0.6, 0.8), labels = c("0.0", "", "0.4", "", "0.8")) +
  theme_classic() +
  theme(axis.text.x = element_text(angle = 50, hjust = 1, size = 14),
        axis.text.y = element_text(size = 14),
        axis.title = element_blank(),
...
p1 <- plot_grid(mobility_occ.gg, mobility_lit.gg, mobility_ded.gg, mobility_wealth.gg, nrow = 2) 

# add the title
title <- ggdraw() + 
  draw_label(
    "Signals of change in social mobility",
    size = 20,
    fontface = 'bold',
    x = 0,
    hjust = 0
...
p2 <- plot_grid(p1, xlabel,
                nrow = 2, 
                rel_heights = c(1, 0.1))

p3 <- plot_grid(ylabel, p2,
                ncol = 2, 
                rel_widths = c(0.05, 1),
                align = "hv")

mobility.gg <- plot_grid(
  title, p3,
  ncol = 1,
...
```{r plot}
fig2.gg <- plot_grid(confound.gg, pseudo.gg, mobility.gg, nrow = 3, ncol = 1, labels = c("(a)", "(b)", "(c)"), rel_heights = c(1,1.1,1), label_size = 20, label_x = 0.0, label_y = c(1.008, 0.965, 0.995))

# add the title
title_main <- ggdraw() + 
  draw_label(
    "Reanalyses of data from Clark (2023)",
    size = 24,
    fontface = 'bold',
    x = 0,
    hjust = 0,
...
ggsave("fig2.png", width = 8, height = 18, units = "in")
ggsave("fig2.pdf", width = 8, height = 18, units = "in")
```
...
Data underlying Figure 3b,c are prepared in `SZ2024.R`; the chunks below are for plotting.

```{r fig 3b}
fig3b_data <- read_tsv("data/fig3b_data.tsv")

fig3b_data.gg <- fig3b_data %>%
  ...
  mutate(trait=ifelse(p_value<0.05/19, paste0(trait, "*"), trait)) %>%
  ...
  geom_point()+
  scale_colour_manual(values=c("#0097a7", "#0044A7"))+
  ...
  xlab("Partial genetic correlation between BSB in males and number of children")+
  # ylab(NULL)+
  ylab("**Measure X**<br>measure for which genetic correlations are being adjusted")+
  coord_cartesian(clip = 'off') +
  theme_classic()+
...
fig3b <- ggdraw(fig3b_data.gg) +
  draw_image("data/path_3b.png", x = -0.05, y = 0.35, width = 0.6, height = 0.6)
```

```{r fig 3c}
fig3c_data <- read_tsv("data/fig3c_data.tsv")
...
fig3c <- fig3c_data %>% 
  ...
  annotate("segment", yend = 0.0218, x = 4.5, xend = 4.5, y = 0.08, colour="black",
  ...
  geom_text(aes(label=paste0("N=", scales::comma(ntot))), size=5, angle=0, hjust= -0.1)+
  ...
  theme(
        axis.title.x=element_markdown(size=18, margin = margin(10,10,10,10)),
        axis.text.x=element_text(size=16),
        axis.title.y=element_text(size=18),
        axis.text.y=element_text(size=16, angle = 45, vjust = 0),
        legend.position="none",
...
```{r plot}
fig3a <- ggdraw() +
  draw_image("data/path_3a.png")

top_row <- plot_grid(fig3a, fig3c, nrow = 1, rel_widths = c(0.8,1), labels = c("(a)", "(c)"), label_size = 20)

#ggsave("fig3.png")

plot_grid(top_row, fig3b, nrow = 2, rel_heights = c(0.8, 1), labels = c("", "(b)"), label_size = 20, label_y = 1.05)

#ggsave("fig3.png", width = 14, height = 13, units = "in")
#ggsave("fig3.pdf", width = 14, height = 13, units = "in")
```
...
Here we explore the tests of environmental influences on social status presented in Clark (2023) Figures 3 and 4.

## Father-son correlations (C23 Fig 4)

First we see how father-son correlations in status measures varies with age at fathers death.
...
color_map <- c("Education - Education" = "#E3211C", "Occupation - Occupation" = "black", "Occupation - Wealth" = "grey", "Education - Wealth" = "pink")
shape_map <- c("Education - Education" = 19, "Occupation - Occupation" = 15, "Occupation - Wealth" = 17, "Education - Wealth" = 18)  # 19 = circle, 15 = square, 17 = triangle


father_son.gg <- father_son_wealth_binned %>%
  filter(measure != "Wealth") %>%
  mutate(measure = fct_recode(measure, `Education - Education` = "Education", `Occupation - Occupation` = "Occupation", `Occupation - Wealth` = "OccupationWealth", `Education - Wealth` = "EducationWealth")) %>%
...
## Maternal / paternal influence (C23 Fig 3)

Here we show that maternal and paternal effects on offspring status measures and wealth are statistically indistinguishable when data used across these three regressions are identical.
...
# clustered CIs, like in Clark Fig 3. Clustering on father
ded_coef_sum <- tidy(coeftest(ded_coef.lm, vcov = vcovCL, cluster = ~ pidpgf), conf.int = T) %>%
  mutate(trait = "Education")
...
# clustered CIs, like in Clark Fig 3. Clustering on father
occ_coef_sum <- tidy(coeftest(occ_coef.lm, vcov = vcovCL, cluster = ~ pidpgf), conf.int = T) %>%
  mutate(trait = "Occupation")
...
# clustered CIs, like in Clark Fig 3. Clustering on father
wealth_coef_sum <- tidy(coeftest(wealth_coef.lm, vcov = vcovCL, cluster = ~ pidpgf), conf.int = T) %>%
  mutate(trait = "Wealth")
...
coefs.gg <- ggplot(coefs.df, aes(x = trait, y = estimate, color = parent, shape = parent)) +
  geom_pointrange(aes(ymin = conf.low, ymax = conf.high), position = position_dodge(width = 0.2), size = 1) +
...
```{r fig S2a}
# Comparing the “lineage-agnostic” correlations in occupational status (1780-1859) that (1) relies on (orange lines) with the distributions of lineage-specific correlations and their medians (blue lines)
...
```{r fig S2bc}
# Lineage-specific correlations in occupational status for first-degree relatives are associated with the representation of the lineage
...
```{r plot S2}
figS2 <- plot_grid(figS2a.gg, figS2bc.gg, ncol = 2)
...
Variation and substructure among and within surname lineages.

```{r among lineage}
# Plot 4th cousin relationships in occupational status for conditioning on surname lineage (nid)
...
```{r surname 1436}
# Plot correlation in occStat across degree of relatedness, for one specific lineage
...
```{r within lineage}
# Plot sublineage structure within a select group of lineages


# get list of lineages with strong 4th cousin correlations in occStat
...
Relationship between lineage-specific correlation in occupational status and lineage size, across relative types.

```{r}
# calculate occStat correlations and sample size for each relationship type in each lineage
occstat_cor_dists <- rels_occ %>%
  dplyr::filter(per1780==1 & relationship!="random") %>%
  group_by(relationship, nid) %>%
  mutate(n=n()) %>%
...
Distributions of familial status correlations stratified by sublineage.
...
Effect of pseudoreplication in Clark (2023) on variance and covariance estimates.

```{r}
# variance and covariance from Clark's data
pr_ss <- rels_occ %>%
  dplyr::filter(per1780==1 & relationship!="random") %>% 
  group_by(relationship) %>% 
...
# variance/covariance after bootstrapping
figS8_data_new1 <- ds_occstat %>%
  dplyr::select(-n) %>%
  merge(relations.df, by.x="relationship", by.y="relationship_rels_occ") %>%
#  mutate(relationship_rename = fct_recode(relationship_rename,
#                                          "Siblings once removed" = "Uncle-nephew",
#                                          "Grandparent-grandchild" = "Grandparent-Grandchild")) %>%
  mutate(n = ifelse(relationship_rename == "Parent-child", 1,
                    ifelse(relationship_rename == "Grandparent-grandchild", 2, n))) %>%
  mutate(relationship_rename = fct_relevel(relationship_rename, relationship_rename_vec)) %>%
  mutate(n_adj = ifelse(relationship_rename == "Parent-child", 0.85, 
                        ifelse(relationship_rename == "Full siblings", 1.15, 
                               ifelse(relationship_rename == "Avuncular", 1.85, 
                                      ifelse(relationship_rename == "Grandparent-grandchild", 2.15, n)))))

# load from file
# figS8_data_new1 <- read.csv("fig_s8_bootstrap_data.tsv", sep = "\t") %>%
#   mutate(cov = as.numeric(cov),
#          var = as.numeric(var),
#          cor = as.numeric(cor))

# covariance plot
s8_cov <- figS8_data_new1 %>%
  ggplot(aes(x=n_adj, y=cov, group=relationship_rename))+
  geom_jitter(alpha=0.3, colour="#5ab4ac")+
  geom_boxplot(alpha=0)+
  geom_point(data=merge(pr_ss %>% dplyr::rename(cov_na=cov), 
                        figS8_data_new1, by="relationship"), 
             aes(y=cov_na), 
             colour="#d8b365", shape=17, size=6)+
  geom_text(data=figS8_data_new1 %>%
              group_by(n_adj) %>%
              filter(cov == max(cov)),
            aes(y = (cov + 5), label=relationship_rename),
            colour="black",
            angle = 90,
            vjust = 0.3,
            hjust = 0,
            lineheight = 0.8)+
  #geom_brace(aes(c(0.8, 1.2), c(-100, -80)), inherit.data = F, rotate = 180) +
  #geom_brace(aes(c(1.8, 2.2), c(-100, -80)), inherit.data = F, rotate = 180) +
  annotate("text", x = 0.8, y = 400, 
           label = "Clark (2023) estimation;\nsusceptible to pseudo-replication", 
           hjust = 0, color = "#d8b365", size = 6) +
  annotate("text", x = 0.8, y = 0, 
           label = "Estimation corrected for\npseudo-replication", 
           hjust = 0, color = "#5ab4ac", size = 6) +
  theme_clark()+
  scale_x_continuous(breaks = 1:9) +
  scale_y_continuous(limits=c(-100, 500)) +
  xlab("Degree of relatedness (n)")+
  ylab("Sample covariance")

# variance plot
s8_var <- figS8_data_new1 %>%
  ggplot(aes(x=n_adj, y=var, group=relationship_rename))+
  geom_jitter(alpha=0.3, colour="#5ab4ac")+
  geom_boxplot(alpha=0)+
  geom_point(data=merge(pr_ss %>% 
                          mutate(var_na=sqrt(var0)*sqrt(var1)), 
                        figS8_data_new1, by="relationship"), aes(y=var_na), 
             colour="#d8b365", shape=17, size=6)+
  geom_text(data=figS8_data_new1 %>%
              group_by(n_adj) %>%
              filter(var == max(var)),
            aes(y = (var + 10), label=relationship_rename),
            colour="black",
            angle = 90,
            vjust = 0.3,
            hjust = 0,
...
plot_grid(s8_cov, s8_var, nrow = 2, labels = c("(a)", "(b)"))

ggsave("fig_s8.png", width = 8, height = 12, units = "in")
```
...
Empirical distributions of occupational status scores vary with genealogical distance and are distorted by pseudoreplication. 
...
# get df for all pairs included in dataset
...
Mean and variance of birth year and status measures across genealogical distance.
...
# calculate mean and variance of occStat, birth year, and wealth across relationship classes
...
Illustrate how the persistence parameter b is insensitive to the magnitude of familial correlations and describes only the rate at which they change as relatedness decreases.
...
occ_stat.df <- relations.df %>%
  ...
  mutate(cor_90 = cor * 0.1) # hypothetical 90% decline in correlation
...
Simulate sensitivity of the persistence parameter, b, to changes in relative correlations. 
