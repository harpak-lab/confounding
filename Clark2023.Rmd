...
#### Reported correlations in Table 2
...
#### Fix these data using updated data from Clark
...
# rels_occ_fam_wealth_long <- rels_occ_fam_wealth %>%
#   dplyr::select(pid0, pid1, occ0, occ1, pid0_father_wealth, pid1_father_wealth) %>%
#   rename(fatherWealth_0 = pid0_father_wealth,
#          fatherWealth_1 = pid1_father_wealth,
#          pid_0 = pid0,
#          pid_1 = pid1,
#          occ_0 = occ0,
#          occ_1 = occ1) %>%
...
# 
# 
# wealth.lm <- lm(occ ~ fatherWealth, data = rels_occ_fam_wealth_long)
# 
# summary(wealth.lm)
# 
# occ0_p <- predict(wealth.lm, newdata = rels_occ_fam_wealth %>% rename(fatherWealth = pid0_father_wealth))
# 
# occ1_p <- predict(wealth.lm, newdata = rels_occ_fam_wealth %>% rename(fatherWealth = pid1_father_wealth))

# predict_occ.df <- rels_occ_fam_wealth %>%
#   mutate(occ0_p = occ0_p,
#          occ1_p = occ1_p)
...
```{r plot Fig 3}
corrected_vs_uncorrected_cors <- read.csv(paste0(data_loc, "corrected_vs_uncorrected_cors.csv")) %>%
  mutate(name = str_replace_all(name, "\n ", "\n"),
           relationship_rename = str_replace_all(relationship_rename, "\n ", "\n")) %>%
...
# brackets for n = 1 and 2
# label_function <- function(x) {
#   ifelse(x == 1 | x == 2, paste0("[", x, "]"), as.character(x))
# }
...
corrected_vs_uncorrected_cors %>%
  ggplot(aes(y=value, x=n_adj, shape=name, color = name, group=name))+
  geom_point(size=5) +
  geom_line(data = line.df, aes(x = n, y = mean_value, color = name, group = name), linewidth = 1) +
  ...
  geom_text(data=corrected_vs_uncorrected_cors %>%
                     group_by(n_adj) %>%
                     filter(value == max(value)),
                   aes(y = 10^(log10(value) + hjust_adjustment), label=relationship_rename),
                   colour="black",
                   angle = 90,
                    vjust = 0.3,
            hjust = 0,
...
write.csv(results_df_occ_perc, "results_df_occ_perc.csv", row.names = F)
...
# mobility_all.df %>%
#   filter(measure == "Literacy") %>%
#   mutate(byr1_bin = fct_relevel(byr1_bin, c("1720-1740", "1740-1760", "1760-1780", "1780-1800", "1800-1820", "1820-1840", "1840-1860", "1860-1880"))) %>%
# ggplot(aes(x = byr1_bin, y = mean_value, ymin = lower_95, ymax = upper_95, color = relationship_rename, group = relationship_rename)) +
#   geom_pointrange(position = position_dodge(width = 0.2)) +
#   geom_line(position = position_dodge(width = 0.2)) +
#   labs(x = "Birth Period",
...
rels_occ_father_son_binned_perc <- rels_occ_father_son %>%
  mutate(byr1_bin = cut(byr1, breaks = seq(1780, 1920, by = 20),
                        labels = paste0(seq(1780, 1920 - 20, by = 20), "-", seq(1800, 1920, by = 20)),
                        include.lowest = TRUE)) %>%
  group_by(byr1_bin) %>%
  do({
    data <- .
    ...
    data$occ0_rank <- calc_percentile_rank(data$occ0)
    data$occ1_rank <- calc_percentile_rank(data$occ1)
    
    # Correlation for original values
    orig_cor <- tidy(cor.test(data$occ0, data$occ1, method = "pearson"))
    
    # Correlation for percentile ranks
    rank_cor <- tidy(cor.test(data$occ0_rank, data$occ1_rank, method = "pearson"))
    
    data.frame(relationship = unique(data$relationship),
               byr1_bin = unique(data$byr1_bin),
               type = c("original", "percentile_rank"),
               estimate = c(orig_cor$estimate, rank_cor$estimate),
               conf.low = c(orig_cor$conf.low, rank_cor$conf.low),
               conf.high = c(orig_cor$conf.high, rank_cor$conf.high))
  }) %>%
  dplyr::select(relationship, byr1_bin, type, estimate, conf.low, conf.high)

rels_occ_father_son_binned_perc %>%
  filter(type == "percentile_rank") %>%
ggplot() +
  geom_pointrange(aes(x = byr1_bin, y = estimate, ymin = conf.low, ymax = conf.high)) +
...
```{r fig S2a}
figs3_dat <- read.csv(paste0(data_loc, "fig3s_dat.csv")) %>%
  mutate(relationship_rename = fct_relevel(relationship_rename, relationship_rename_vec))

figs3_dat$group <- gsub("\\\\n", "\n", figs3_dat$group)
...
fig3a.gg <- occstat_cor_dists %>%
  ...
  geom_text_repel(data = figs3_dat %>% 
...
ggsave("fig_s3.png", height = 12, width = 22, units = "in")
...
# Fig S5 - Cor ~ N
```{r}
occstat_cor_dists <- read.csv(paste0(data_loc, "occstat_cor_dists.csv")) %>%
  mutate(relationship_rename = fct_relevel(relationship_rename, c("Full siblings", "Parent-child", "Uncle-nephew", "Grandparent-Grandchild", "1st Cousins", "1st Cousins\n once removed", "2nd Cousins", "2nd Cousins\n once removed", "3rd Cousins", "3rd Cousins\n once removed", "4th Cousins")))

occstat_cor_dists_nr <- read.csv(paste0(data_loc, "occstat_cor_dists_nr.csv")) 

occstat_rho_vs_n <- read.csv(paste0(data_loc, "occstat_rho_vs_n.csv")) %>%
  mutate(relationship_rename = fct_relevel(relationship_rename, c("Full siblings", "Parent-child", "Uncle-nephew", "Grandparent-Grandchild", "1st Cousins", "1st Cousins\n once removed", "2nd Cousins", "2nd Cousins\n once removed", "3rd Cousins", "3rd Cousins\n once removed", "4th Cousins")))


occstat_cor_dists %>%
  dplyr::select(-occstat) %>%
  merge(occstat_cor_dists_nr, by="nid") %>%
  #dplyr::filter(rel2 %in% c("Full siblings", "Parent-child")) %>%
  ggplot(aes(x=n.x, y=cor.x))+
...
for (i in 1:num_sims) {
  ...
  # Use the linear model to generate cor[2:8] values
  n_values <- 2:8
...
b1.gg <- ggplot(df, aes(x = n_9, y = b)) +
  geom_point() +
#  geom_vline(aes(xintercept = 0.06),color = "red", size = 2) +
#  annotate("text", x = 0.082, y = 0.6, label = "Mean correlation\ndue to inflation", color = "red", size = 5) +
